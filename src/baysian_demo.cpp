...
*/
        ...
        Mat teddy = imread("../test_images/thinput.png");
        imshow("demo_source_image", teddy);
        Mat trimap = imread("../test_images/thmask.png");
        imshow("trimap", trimap);
        BayesianMatting b_matting;
        Mat alpha = b_matting.Process(teddy, trimap);
        imshow("after_matting", alpha);
    ...
    else
    {
        ...
        if(save_video)
        {
            ...
            writer.open("../videos/output2.avi", fourcc, 24, Size(960, 540));
        }
        while (1)
        {
